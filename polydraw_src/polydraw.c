...
static void printg_init (void)
{
	int j, x, y, xsiz, ysiz, *iptr, *tbuf;

		//Load 6x8(x256) font
	xsiz = 8; ysiz = 8*256;
	if (!(tbuf = (int *)malloc(xsiz*ysiz*4))) return;
	for(y=0,iptr=tbuf;y<ysiz;y++,iptr+=xsiz)
		for(x=0;x<xsiz;x++)
		{
			if (x < 6) { if (((char *)font6x8)[(y>>3)*6+(x&7)]&(1<<(y&7))) j = -1; else j = 0; }
					else j = 0;
			iptr[x] = j;
		}
...
static double myprintg (double dx, double dy, double dfcol, char *fmt, ...)
{
	...
	x = (int)dx; y = (int)dy; fcol = (int)dfcol;
	...
	else { glDisable(GL_VERTEX_PROGRAM_ARB); glDisable(GL_FRAGMENT_PROGRAM_ARB); }
	if (glfp[glActiveTexture]) ((PFNGLACTIVETEXTUREPROC)glfp[glActiveTexture])(GL_TEXTURE0);
	...
	{
		if (intab) intab--; else ich = *cptr++;
		if (ich == 9) { intab = 2; ich = ' '; }

		glTexCoord2f(0.f ,((float) ich   )/256.f); glVertex2i(x  ,y  );
		glTexCoord2f(.75f,((float) ich   )/256.f); glVertex2i(x+6,y  );
		glTexCoord2f(.75f,((float)(ich+1))/256.f); glVertex2i(x+6,y+8);
		glTexCoord2f(0.f ,((float)(ich+1))/256.f); glVertex2i(x  ,y+8);
	}
	glEnd();
	...
	if ((usearbasm) || (usearbasmonly)) { glEnable(GL_VERTEX_PROGRAM_ARB); glEnable(GL_FRAGMENT_PROGRAM_ARB); }
	return(0.0);
}
