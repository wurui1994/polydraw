...
static void *kasm87comp(char *bakz)
{
	...
	if (!memnum)
		...
		espoff = (memnum << 3) + 8; // Offset ESP (passed params) by temp register size
	for (i = gecnt - 1; i >= 0; i--)
		for (j = gasm[i].n; j >= 0; j--)
		{
			if (j < 3)
			...
			if ((rp->r & 0xf0000000) == KESP)
				rp->r += espoff;
			else if ((rp->r & 0xf0000000) == KPTR)
				rp->r += espoff;
			else if ((rp->r & 0xf0000000) == KECX)
			{
				if ((rp->r & 0x0fffffff) < (4 << 3))
					rp->r += (KFST - KECX);
				else
					rp->r += (KESP - KECX) - (4 << 3);
			}
	...
	for (putwrite = 0; putwrite < 2; putwrite++)
	{
		...
		for (i = 1; i < gecnt; i++)
		{
			...
			case GOTO:
				...
				put1byte(0xe9);
				put4byte(gasm[i].r[1].r & 0x0fffffff); // jmp ?
				checklabs(jumpatnum + 1);
				jumpat[jumpatnum++] = kasm87leng;
				if (putwrite)
				{
					checkjumpbacks(jumpbacknum + 1);
					jumpback[jumpbacknum++].addr = (long)&compcode[kasm87leng - 4];
				}
				else
					ljumpbacknum++;
				break;
			case RETURN:
				...
				for (j = regnum; j > 0; j--)	 /*if (fpustat&(1<<(j-1)))*/
					put2byte(0xc0dd + (j << 8)); // ffree st(j) (ffree faster on P4: less dependency?)
			...
			case NEQU0: // gvl[gasm[i].r[0]] = gvl[gasm[i].r[1]] != 0
				if (((gasm[i].r[1].r & 0xf0000000) == KFST) || ((gasm[i].r[1].r & 0xf0000000) == KPTR) || ((gasm[i].r[1].r & 0xf0000000) == KIMM) || ((gasm[i].r[1].r & 0xf0000000) == KGLB))
				{
					...
					if (!(cputype & (1 << 15))) // no CMOV
					{
						...
						put2byte(0xe480);
						put1byte(0x40); // and ah, 0x40
						put1byte(0x75);
						put1byte(4); // jnz short ?
					}
					else
					{
						put2byte(0xeed9); // fldz
			...
			case IF0:
				if (((gasm[i].r[2].r & 0xf0000000) == KFST) || ((gasm[i].r[2].r & 0xf0000000) == KPTR) || ((gasm[i].r[2].r & 0xf0000000) == KIMM) || ((gasm[i].r[2].r & 0xf0000000) == KGLB))
				{
					...
					if (!(cputype & (1 << 15))) // no CMOV
					{
						...
						put1byte(0x40); // and ah, 0x40
						put2byte(0x850f);
						put4byte(gasm[i].r[1].r & 0x0fffffff); // jnz ?
					}
					else
					{
						put2byte(0xeed9); // fldz
						...
						put2byte(0xd8dd); // fstp st(0)
						put2byte(0x840f);
						put4byte(gasm[i].r[1].r & 0x0fffffff); // jz ?
					}
				}
				else
				{
			...
			case RND: // gvl[gasm[i].r[0]] = ((double)krand())/2^31; break;
					  // 31-bit RND
				put1byte(0xa1);
				put4byte((long)&kholdrand); // mov eax, kholdrand
				put2byte(0xc069);
				put4byte(214013 * 2); // imul eax, 214013*2
				put1byte(0x05);
				put4byte(2531011 * 2); // add eax, 2531011*2
				put2byte(0xe8d1);	   // shr eax, 1
				put1byte(0xa3);
				put4byte((long)&kholdrand); // mov kholdrand, eax
				put2byte(0x05db);
				put4byte((long)&kholdrand); // fild dword ptr [kholdrand]
				put2byte(0x0dd8);
				put4byte((long)&oneover2_31); // fmul dword ptr [oneover2_31]
				break;
			case NRND:			// gvl[gasm[i].r[0]] = nrnd(); break;
				put1byte(0x52); // push edx

				put1byte(0xb8);
				put4byte((long)nrnd); // mov eax, offset nrnd //use this to allow code dup
			...
			case POW: // gvl[gasm[i].r[0]] = pow(gvl[gasm[i].r[1]],gvl[gasm[i].r[2]]); break; //POW: x ^ y = 2^(LOG2(x)*y)
				...
				put2byte(0xec83);
				put1byte(16); // sub esp, 16
				put1byte(0xdd);
				put2byte(0x241c);	  // fstp qword ptr [esp]
				put4byte(0x08245cdd); // fstp qword ptr [esp+8]

				put1byte(0xb8);
				put4byte((long)kpow); // mov eax, offset kpow //use this to allow code dup
				put2byte(0xd0ff);	  // call eax
				// put1byte(0xe8); put4byte(((long)kpow)-((long)&compcode[kasm87leng+4])); //call kpow

				put2byte(0xc483);
				put1byte(16); // add esp, 16
#endif
				break;
			case TIMES: // gvl[gasm[i].r[0]] = gvl[gasm[i].r[1]]*gvl[gasm[i].r[2]]; break;
				...
				{
					if (gasmeq(gasm[i - 1].r[0], gasm[i].r[1]))
						j = 1;
					else
						j = 2;
			...
			case PERC:							  // gvl[gasm[i].r[0]] = gvl[gasm[i].r[1]]-floor(gvl[gasm[i].r[1]]/gvl[gasm[i].r[2]])*gvl[gasm[i].r[2]]; break;
				...
				put2byte(0xe0df);				  // fnstsw ax
				put2byte(0xe480);
				put1byte(0x01); // and ah, 0x01
				put1byte(0x74);
				put1byte(6);	  // jz short skip
			...
			case SGN:						// if (gvl[gasm[i].r[1]] < 0) gvl[gasm[i].r[0]] =-1.0; else if (gvl[gasm[i].r[1]] > 0) gvl[gasm[i].r[0]] = 1.0; else gvl[gasm[i].r[0]] = 0.0; break;
				...
				put1byte(0x74);
				put1byte(8); // jz short skip1
				put2byte(0x05d9);
				put4byte((long)&negone); // fld dword ptr [negone]
				put1byte(0xeb);
				put1byte(12);		  // jmp short endit
				put4byte(0x4000a966); // skip1: test ax, 0x4000
				put1byte(0x74);
				put1byte(4);	  // jz short skip2
				put2byte(0xeed9); // fldz
				put1byte(0xeb);
				put1byte(2);	  // jmp short endit
			...
			case UNIT:						// if (gvl[gasm[i].r[1]] < 0) gvl[gasm[i].r[0]] = 0.0; else if (gvl[gasm[i].r[1]] > 0) gvl[gasm[i].r[0]] = 1.0; else gvl[gasm[i].r[0]] = 0.5; break;
				...
				put4byte(0x0100a966);		// test ax, 0x0100
				put1byte(0x74);
				put1byte(4);	  // jz short skip1
				put2byte(0xeed9); // fldz
				put1byte(0xeb);
				put1byte(16);		  // jmp short endit
				put4byte(0x4000a966); // skip1: test ax, 0x4000
				put1byte(0x74);
				put1byte(8); // jz short skip2
				put2byte(0x05d9);
				put4byte((long)&pointfive); // fld dword ptr [pointfive]
				put1byte(0xeb);
				put1byte(2);	  // jmp short endit
				put2byte(0xe8d9); // skip2: fld1
				break;
			...
			case CEIL:						// gvl[gasm[i].r[0]] = ceil(gvl[gasm[i].r[1]]); break;
				...
				break;
			case ROUND0:
			case ROUND0_32:					// gvl[gasm[i].r[0]] = round0(gvl[gasm[i].r[1]]); break;
				put1stfld(i, gasm[i].r[1]); // fld qword ptr [?]
				if (!(cputype & (1 << 27))) // no SSE3
				{
					...
					put1byte(8); // sub esp, 8
					put2byte(0x1cdd);
					put1byte(0x24);		  // fstp qword ptr [esp]
					put4byte(0x0424448b); // mov eax, dword ptr [esp+4]
					put1byte(0x52);		  // push edx
					put2byte(0xd08b);	  // mov edx, eax
					put1byte(0x25);
					put4byte(0x7ff00000); // and eax, 0x7ff00000
					put2byte(0xe8c1);
					put1byte(17); // shr eax, 17
					put2byte(0x9023);
					put4byte(((long)round0msk) + 4); // and edx, dword ptr round0msk[eax+4]
					put4byte(0x08245489);			 // mov dword ptr [esp+8], edx
					put4byte(0x0424548b);			 // mov edx, dword ptr [esp+4]
					put2byte(0x9023);
					put4byte((long)round0msk); // and edx, dword ptr round0msk[eax]
					put4byte(0x04245489);	   // mov dword ptr [esp+4], edx
					put1byte(0x5a);			   // pop edx
					put2byte(0x04dd);
					put1byte(0x24); // fld qword ptr [esp]
					put2byte(0xc483);
					put1byte(8); // add esp, 8
				}
				else
				{
					...
					{
						put2byte(0xec83);
						put1byte(4); // sub esp, 4
						put2byte(0x0cdb);
						put1byte(0x24); // fisttp dword ptr [esp]
						put2byte(0x04db);
						put1byte(0x24); // fild qword ptr [esp]
						put2byte(0xc483);
						put1byte(4); // add esp, 4
					}
					else
					{
						put2byte(0xec83);
						put1byte(8); // sub esp, 8
						put2byte(0x0cdd);
						put1byte(0x24); // fisttp qword ptr [esp]
						put2byte(0x2cdf);
						put1byte(0x24); // fild qword ptr [esp]
						put2byte(0xc483);
						put1byte(8); // add esp, 8
			...
			case MIN:							  // if (gvl[gasm[i].r[2]] < gvl[gasm[i].r[1]]) gvl[gasm[i].r[0]] = gvl[gasm[i].r[2]]; else gvl[gasm[i].r[0]] = gvl[gasm[i].r[1]]; break;
				...
				put2byte(0xe0df);				  // fnstsw ax
				put2byte(0xe480);
				put1byte(0x41); // and ah, 0x41
				put1byte(0x75);
				put1byte(putlen(gasm[i].r[2]) + 3); // jnz short skip
				putsib(0xdd, 0x00, gasm[i].r[2]);	// fld qword ptr [?]
				put1byte(0xeb);
				put1byte(putlen(gasm[i].r[1]) + 1); // jmp short endit
				putsib(0xdd, 0x00, gasm[i].r[1]);	// fld qword ptr [?]
				break;
			case MAX:							  // if (gvl[gasm[i].r[2]] > gvl[gasm[i].r[1]]) gvl[gasm[i].r[0]] = gvl[gasm[i].r[2]]; else gvl[gasm[i].r[0]] = gvl[gasm[i].r[1]]; break;
				...
				put1byte(0x41); // and ah, 0x41
				put1byte(0x74);
				put1byte(putlen(gasm[i].r[2]) + 3); // jz short skip
				putsib(0xdd, 0x00, gasm[i].r[2]);	// fld qword ptr [?]
				put1byte(0xeb);
				put1byte(putlen(gasm[i].r[1]) + 1); // jmp short endit
				putsib(0xdd, 0x00, gasm[i].r[1]);	// fld qword ptr [?]
			...
			case EXP:							  // gvl[gasm[i].r[0]] = exp(gvl[gasm[i].r[1]]); break;
				...
				put1byte(8); // sub esp, 8
				put2byte(0x14db);
				put1byte(0x24); // fist dword ptr [esp]
				put2byte(0x24da);
				put1byte(0x24); // fisub dword ptr [esp]
				put2byte(0x0481);
				put1byte(0x24);
				put4byte(0x3fff); // add dword ptr [esp], 0x3fff
				put2byte(0xf0d9); // f2xm1
				put2byte(0x05d8);
				put4byte((long)&posone); // fadd dword ptr [posone]
				put1byte(0x68);
				put4byte(0x80000000); // push 0x80000000
				put2byte(0x006a);	  // push 0
				put2byte(0x2cdb);
				put1byte(0x24);	  // fld tbyte ptr [esp]
				put2byte(0xc9de); // fmulp st(1), st(0)
				put2byte(0xc483);
				put1byte(16); // add esp, 16
#endif
			...
			case FACT:						// gvl[gasm[i].r[0]] = fact(gvl[gasm[i].r[1]]); break;
				...
				put1byte(0xb8);
				put4byte((long)fact); // mov eax, offset fact //use this to allow code dup
				put2byte(0xd0ff);	  // call eax
				// put1byte(0xe8); put4byte(((long)fact)-((long)&compcode[kasm87leng+4])); //call fact

				put2byte(0xc483);
				put1byte(0x08); // add esp, 8
				break;
			...
			case PEEK:						// gvl[gasm[i].r[0]] = *(double *)(((long)&gvl[gasm[i].r[1]])+gvl[gasm[i].r[2]]*8); break;
				...
				else
				{
					...
					put1byte(8); // sub esp, 8
					put2byte(0x14db);
					put1byte(0x24); // fist dword ptr [esp]   (rounded value)
					put2byte(0x24da);
					put1byte(0x24);		  // fisub dword ptr [esp]  (sub rounded value)
					put4byte(0x04245cd9); // fstp dword ptr [esp+4] (diff)
					put1byte(0x58);		  // pop eax                (rounded value)
					put1byte(0x59);		  // pop ecx                (diff (float))
					put2byte(0xc181);
					put4byte(0x7fffffff); // add ecx, 0x7fffffff    (set CF if diff < -0)
					put2byte(0xd883);
					put1byte(0); // sbb eax, 0             (dec if x-round(x) < -0)
				}
				...
				{
					put2byte(0x04dd);
					put1byte(0xc5); // fld qword ptr [eax*8+imm32]
					tr = gasm[i].r[1];
					if (putwrite)
					{
						checkpatch(patchnum + 1);
				...
				else
				{
					...
					put2byte(0x04dd);
					put1byte(0xc1); // fld qword ptr [eax*8+ecx]
				}
				break;
			...
			case LAND: // gvl[gasm[i].r[0]] = gvl[gasm[i].r[1]] && gvl[gasm[i].r[2]];
					...
					((gasm[i].r[2].r & 0xf0000000) == KGLB))
				{
					...
					put1byte(0x40); // and ah, 0x40
					put1byte(0x75);
					put1byte(putlen(gasm[i].r[2]) + 16); // jnz short ?
					putsib(0xdd, 0x00, gasm[i].r[2]);	 // fld qword ptr [?]
					put2byte(0xe4d9);					 // ftst
					put2byte(0xe0df);					 // fnstsw ax
					put2byte(0xd8dd);					 // fstp st(0)
				...
				else
				{
					...
					tr.r += 4;
					put1byte(0x74);
					put1byte(putlen(gasm[i].r[2]) + putlen(tr) + 8); // jz short skip
					putsib(0x8b, 0x00, gasm[i].r[2]);				 // mov eax, dword ptr [?]
			...
			case LOR: // gvl[gasm[i].r[0]] = gvl[gasm[i].r[1]] || gvl[gasm[i].r[2]];
					...
					((gasm[i].r[2].r & 0xf0000000) == KGLB))
				{
					...
					put1byte(0x40); // and ah, 0x40
					put1byte(0x74);
					put1byte(putlen(gasm[i].r[2]) + 16); // jz short ?
					putsib(0xdd, 0x00, gasm[i].r[2]);	 // fld qword ptr [?]
					put2byte(0xe4d9);					 // ftst
					put2byte(0xe0df);					 // fnstsw ax
					put2byte(0xd8dd);					 // fstp st(0)
				...
				if (((gasm[i].r[1].r & 0xf0000000) == KIMM) || ((gasm[i].r[1].r & 0xf0000000) == KGLB))
				{
					...
					else if (gasm[i].f != POKE)
					{
						...
						case POKETIMES:
							put2byte(0x0cdc);
							put1byte(0xc5);
							break; // fmul qword ptr [eax*8+imm32]
						case POKESLASH:
							put2byte(0x3cdc);
							put1byte(0xc5);
							break; // fdivr qword ptr [eax*8+imm32]
						case POKEPLUS:
							put2byte(0x04dc);
							put1byte(0xc5);
							break; // fadd qword ptr [eax*8+imm32]
						case POKEMINUS:
							put2byte(0x2cdc);
							put1byte(0xc5);
							break; // fsubr qword ptr [eax*8+imm32]
						}
						tr = gasm[i].r[1];
						if (putwrite)
						{
					...
					put2byte(0x1cdd);
					put1byte(0xc5); // fstp qword ptr [eax*8+imm32]
					tr = gasm[i].r[1];
					if (putwrite)
					{
						checkpatch(patchnum + 1);
				...
				else
				{
					...
					else if (gasm[i].f != POKE)
					{
						...
						case POKETIMES:
							put2byte(0x0cdc);
							put1byte(0xc1);
							break; // fmul qword ptr [eax*8+ecx]
						case POKESLASH:
							put2byte(0x3cdc);
							put1byte(0xc1);
							break; // fdivr qword ptr [eax*8+ecx]
						case POKEPLUS:
							put2byte(0x04dc);
							put1byte(0xc1);
							break; // fadd qword ptr [eax*8+ecx]
						case POKEMINUS:
							put2byte(0x2cdc);
							put1byte(0xc1);
							break; // fsubr qword ptr [eax*8+ecx]
						}
					}
					put2byte(0x1cdd);
					put1byte(0xc1); // fstp qword ptr [eax*8+ecx]
				}
				break;
			case USERFUNC:
				// put1stfld(i,gasm[i].r[1]);             //fld qword ptr [?]
				...
				for (j = 0, l = 0; j < gasm[i].n; j++)
				{
					...
					else //'C','D'
					{
						if ((tr.r & 0xf0000000) == KEDX)
						{
							...
							{
								strcpy(kasm87err, "ERROR: pointer to constant");
								return (0);
							}
							...
							{
								strcpy(kasm87err, "ERROR: bad string");
								return (0);
							}
						}
						if ((tr.r & 0xf0000000) == KFST)
							tr.r = (tr.r & 0x0fffffff) + k + KESP;
				...
				{
					l = (j << 3) + k;
					put1byte(0xdd); // fld qword ptr [esp+l]
					if (l < 128)
					{
						put2byte(0x2444);
						put1byte(l);
		...
		while (kasm87leng & 15)
			put1byte(0x90); // ALIGN 16 for code blocks
		...
		if ((gccnt) || (gstnum) || (arrnum)) // transplant constant table to after code
		{
			...
			else
			{
				...
				for (i = arrnum & 7; i > 0; i--)
					put1byte(0); // Align 8 for DATA blocks (shouldn't happen unless I implement non-double types)
			}
		}
...
void dumpfp()
{
	...
	_asm fnstenv fpreg if ((fpreg[1] & 64) || ((fpreg[2] & 0xffff) != 0xffff))
	{
